/*
 * doublePriorityQueue.h
 *
 *  Created on: Nov 23, 2016
 *      Author: cochez
 */

#ifndef DOUBLEPRIORITYQUEUE_H_
#define DOUBLEPRIORITYQUEUE_H_

#include "limits.h"
#include "bd.h"

//Copied from priorityQueue.h in Snap/glib-core.
//The original used floats for priority, while we need more precision
...
public:
	...
	void Insert(const TVal& X, double Priority) {
	...
	void SetPriority(const TVal& X, double NewPriority) {
	...
	double GetPriority(const TVal& X) {
		if (ValToIndex.IsKey(X)) {
